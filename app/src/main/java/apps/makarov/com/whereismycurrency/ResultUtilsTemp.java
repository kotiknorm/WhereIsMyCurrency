...
public class ResultUtilsTemp {
    ...
    public static String getDiffStrRateValue(ResultOperation operation, Rate rate){
        ...
        double diff = getRound(rate.getValue()) - getRound(operation.getExitRate().getValue()) ;

        return diff > 0 ? "+" + getRound(diff) : diff + "";
    }

    public static double getRound(double value){
        return BigDecimal.valueOf(value).setScale(5, BigDecimal.ROUND_HALF_DOWN).doubleValue();
    }
}
